...
async function buildQuiz(text) {
    ...
    // Try Chrome AI first
    try {
        if ('ai' in self && self.ai && self.ai.languageModel) {
            console.log('🔍 Checking Chrome AI availability...');
            const availability = await self.ai.languageModel.capabilities();
            console.log('📊 AI availability:', availability.available);

            if (availability.available === 'readily' || availability.available === 'after-download') {
